...
local function highlight_line(line, linenr)
    ...
    -- Break input into UTF8 code points.
      -- ASCII code points (from 0x00 to 0x7f)  can be represented in one byte.
      -- Higher code point  is represented by a leading byte (0xc0 and above)
                                            -- and continuation bytes (0x80 to 0xbf, or decimal 128 to 191).
...
local function highlight_man_page()
    ...
    vim.api.nvim_buf_set_lines(0, 0, -1, false, lines)
                           -- current buffer
                                -- fisrt line
                                    -- last line
                                        -- out-of-bounds should NOT be an  error.
