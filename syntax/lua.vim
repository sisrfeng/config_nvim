...
if exists("b:current_syntax")
    echom "b:current_syntax 是: "   b:current_syntax
    finish
en
...
" Comments
    syn match luaNotEqual  #\~=#  conceal cchar=≠
    ...
    syn match   luaCom_DelI    "-- " contained conceal
    syn match   luaCom_DelI    "--\ze\S"  contained conceal
    "\ Lines in the main description start with `---`:
        syn match   luaCom_DelI    "--- " contained conceal
        syn match   luaCom_DelI    "---\ze\S"  contained conceal

    if lua_version == 5 && lua_subversion == 0
        syn region luaComment
                        \ matchgroup=luaComment
                        \ start="--\[\["
                        \ end="\]\]"
                        \ contains=luaTodo,luaInnerComment,@Spell

        syn region luaInnerComment
                        \ contained
                        \ transparent
                        \ start="\[\["
                        \ end="\]\]"
    ...
    elseif lua_version > 5 || (lua_version == 5 && lua_subversion >= 1)
        ...
        syn region luaComment
                        ...
                        \ start="--\[\z(=*\)\["
                          \ end="\]\z1\]"
                        \ contains=luaTodo,@Spell,@In_fancY,luaCom_DelI
        "\ when we want to comment out a piece of code,
            "\ A common trick  is to write the following:
            "\     --[[
            ...
            "    ¿---[[¿ does not start a block comment;
            "    so, the print is outside comments.
            "    The last line becomes an independent comment,
            "    as it starts with --.

            "\
            "\ Lua documentation lives in the source code
            "\     as docstrings on the function definitions.
                    "\ The |lua-vim| ¿help¿  is ¿generated from the docstrings¿.
                    "\
                    "\ Docstring format:
                    "\     - Lines in the main description start with `---`
                    "\     - Special ¿tokens¿ start with `---@` followed by the token name:
                    "\                 `---@see`, `---@param`, `---@returns`
                    "\     - Limited markdown is supported.  Use `<pre>`  for code samples.
...
syn match Lua_funC #function#  contained containedin=luaFunctionBlock conceal cchar=£
syn match Lua_enD  #^\s*\zsend$#       contained containedin=luaFunctionBlock conceal cchar=♩

syn region luaFunctionBlock
              \ transparent
              "\ \ matchgroup=luaFunction  导致我定义Lua_funC无效
                  \ start="\<function\>"
                  \ end="\<end\>"
              \ contains=ALLBUT,
                \luaTodo,
...
" repeat ... until
syn region luaLoopBlock
                \ transparent
                \ matchgroup=luaRepeat
                \ start="\<repeat\>"
                \ end="\<until\>"
                \ contains=ALLBUT,
                  \luaTodo,
...
syn region luaLoopBlock
                ...
                \ start="\<while\>"
                \ end="\<do\>"me=e-2
                \ nextgroup=luaBlock skipwhite skipempty
                \ contains=ALLBUT,
                  \luaTodo,
                  \luaSpecial,
                  \luaIfThen,
...
" other keywords
syn keyword luaStatement
                \ return
                \ break

syn match   luaLocaL "local " conceal
...
syn match luaConcat #\.\.# conceal cchar=ߙ
...
let b:current_syntax = "lua"   | let &cpo = s:cpo_save  | unlet s:cpo_save
