...
aug  filetypedetect
    au! BufNewFile,BufRead     *.man         setf man
        ...
        "\ todo:
            ...
            " au! BufNewFile,BufRead     *.manu   setf man
            "\ 就不用顾虑.man的filetype被设为nroff?
    au! BufNewFile,BufRead     $HOME/PL/*/doc/*.txt           setf help
    au! BufNewFile,BufRead     $HOME/PL/help_me/vim9/*.txt    setf help
    "\ 有效, 参考了:
...
"\ 之前用本文件架空了$RUNTIME下的同名文件, 现在全部注释掉, 仅用于备份 ==================================
    ...
    "\
    "\ "\ did_load_filetypes  是global variable?
    "\ if exists("did_load_filetypes")  | finish  | en
    "\ let did_load_filetypes = 1
    "\     "\ 原本,
    "\     "\ The ":filetype on" command will load this file:
    "\             "\ $VIMRUNTIME/filetype.vim (2000多行)
    "\     "\ 现在
    "\         "\ let did_load_filetypes = 1 这把它架空了
    "\
    "\ "\ it's better not to modify the $VIMRUNTIME/filetype.vim  file.
    "\ "\ It will be overwritten when installing a new version of Vim.
    "\ "\ 我现在不怕被overwritten, 但担心和后续的update冲突
    "\
    "\
    ...
    "\
    "\ " Bazel (http://bazel.io)
    "\ au BufRead,BufNewFile *.bzl,*.bazel,WORKSPACE   setf bzl
    "\ if has("fname_case")
    "\     " There is another check for BUILD further below.
    "\     au BufRead,BufNewFile *.BUILD,BUILD       setf bzl
    "\ en
    "\
    "\ " C or lpc
    ...
    "\
    "\ " Gift (Moodle)
    "\ au BufRead,BufNewFile *.gift        setf gift
    "\
    "\ " Git
    ...
    "\     "\ man的内容由groff/nroff处理, 但我基本不直接写nroff
    "\
    "\ " Man config
    ...
    "\ en
    "\
    "\ " Function called for testing all functions defined here.
    "\ " These are script-local,
    "\ " thus need to be executed here.
